...
<body>
    ...
    <div id="corpo">
        ...
        if (!isset($_POST['cnpj'])) {
            ...
            $cadastro = new Cadastro($cnpj, $email, $senhaHash, $nomeFantasia, $razaoSocial, $tipoOng);
            $ok = $cadastro->retornarDadosEnvio();
            if ($ok) {
                $q = $ok;
                if ($banco->query($q)) {
                    ...
                    $retur = "SELECT nomeFantasia, razaoSocial, senha, tipoOng, idUsuario, cnpj FROM usuario WHERE email = '$email' LIMIT 1";
                    ...
                    if ($busca) {
                        if ($busca->num_rows > 0) {
                            ...
                            sleep(2);
                            header('Location: ' . 'http://localhost/ponte/perfil/new-profile.php');
                        }
                    } else {
